    ...
    for tgt_file in args.target_path.iterdir():
        with h5py.File(tgt_file, 'r') as target, h5py.File(
          args.predictions_path / tgt_file.name, 'r') as recons:
            ...
            # recons = recons / target.max()
            # recons = recons * np.sqrt(np.prod(recons.shape[1:]))
            # recons = recons*(target.max()/recons.max())
            ...
            # print(target.max(), recons.max(), target.shape, recons.shape)
            # print(target.min(),recons.min())
            # print(target.dtype,recons.dtype)

            # import matplotlib.pyplot as plt
            # import colorcet as cc
            # plt.subplot(1, 2, 1)
            # plt.imshow(np.abs(target[20]).real, cmap=cc.m_gray)
            # plt.title('target')
            # plt.subplot(1, 2, 2)
            # plt.imshow(np.abs(recons[20]).real, cmap=cc.m_gray)
            # plt.title('recon')
            # plt.show()
